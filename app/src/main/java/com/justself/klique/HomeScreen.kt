    ...
    Column(
        ...
        OutlinedTextField(
            ...
            onValueChange = { newText ->
                if (newText.text.length <= maxTopicLength) {
                    topic = newText; showTopicError = newText.text.length < minTopicLength
                } else {
                    showTopicError = true
                }
            },
        ...
        OutlinedTextField(
            value = description,
            onValueChange = { newText ->
                if (newText.text.length <= maxDescriptionLength) {
                    description = newText
                    showDescriptionError = newText.text.isNotEmpty() && newText.text.length < minDescriptionLength
                } else {
        ...
        Row {
            ...
            Text(
                text = "Public",
                modifier = Modifier.clickable { selectedType = "public" },
                color = MaterialTheme.colorScheme.onPrimary
            )
            ...
            Text(
                text = "Private",
                modifier = Modifier.clickable { selectedType = "private" },
                color = MaterialTheme.colorScheme.onPrimary
            )
        }
        ...
        // Submit button
        Button(onClick = {
            val isTopicValid = topic.text.length in minTopicLength..maxTopicLength
            val isDescriptionValid = description.text.length in minDescriptionLength..maxDescriptionLength
            if (isTopicValid && isDescriptionValid) {
                onSubmit(topic.text, selectedType, description.text)
            } else {
                showTopicError = !isTopicValid
        ...
        if (showTopicError) {
            Text(
                "Topic must be at least $minTopicLength characters",
